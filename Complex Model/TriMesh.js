...
/*
    Finished in Lab 6, in this time we try to make 
    it in use to help finishing the mp3 
 */
 ...
 */
    ...
    */
    loadFromOBJ(fileText)
    {
        ...
        var splitL = fileText.split("\n");

        for(var i = 0; i < splitL.length; i++){
            var perLine = splitL[i].split(" ");

            // signal of the split
            var char1 = splitL[i][0];

            if(char1 == "v"){
                // vertex data
                for(var j = 1; j < 4; j++){
                    this.vBuffer.push(parseFloat(perLine[j]));
                }
                this.numVertices++;
            }
            ...
            if(char1 == "f"){
                // fragment segment
                for(var j = 2; j < 5; j++){
                    this.fBuffer.push(parseInt(perLine[j]) - 1);
                }
                this.numFaces++;
            }
        }
